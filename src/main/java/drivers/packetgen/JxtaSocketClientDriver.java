...
public class JxtaSocketClientDriver { 
	...
	public JxtaSocketClientDriver(boolean waitForRendezvous) {
		try {
			...
			String address = null;
			if(rdvAddress == null)
				address = "tcp://" + InetAddress.getLocalHost().getHostAddress() + ":" + JxtaSocketServerDriver.tcpPort;
			else
	...
	public void run(int size) {
		try {
			...
			JxtaSocket socket = new JxtaSocket(netPeerGroup, null, JxtaSocketServerDriver.createSocketAdvertisement(), 60000, true);
	...
	public static void main(String args[]) {
		...
		try {
			Thread.currentThread().setName(JxtaSocketClientDriver.class.getName() + ".main()");			
			boolean waitForRendezvous = true;
			JxtaSocketClientDriver jxtaPeer = new JxtaSocketClientDriver(waitForRendezvous);
